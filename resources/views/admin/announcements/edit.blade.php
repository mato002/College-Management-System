                ...
                <form method="post" action="{{ route('admin.announcements.update', $announcement) }}">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title', $announcement->title) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                            @error('title')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label for="body" class="block text-sm font-medium text-gray-700">Content</label>
                            <textarea name="body" id="body" rows="5" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">{{ old('body', $announcement->body) }}</textarea>
                        ...
                        <div>
                            ...
                            <select name="scope" id="scope" class="mt-1 block w-full rounded-md border-gray-300" onchange="document.getElementById('unit_id_group').style.display = this.value === 'unit' ? 'block' : 'none'">
                                <option value="global" {{ old('scope', $announcement->scope) === 'global' ? 'selected' : '' }}>Global</option>
                                <option value="unit" {{ old('scope', $announcement->scope) === 'unit' ? 'selected' : '' }}>Unit-specific</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="type" id="type" class="mt-1 block w-full rounded-md border-gray-300">
                                <option value="news" {{ old('type', $announcement->type ?? 'news') === 'news' ? 'selected' : '' }}>News & Updates</option>
                                <option value="notice" {{ old('type', $announcement->type ?? 'news') === 'notice' ? 'selected' : '' }}>Notice / Announcement</option>
                            </select>
                        </div>
                        <div id="unit_id_group" style="display: {{ old('scope', $announcement->scope) === 'unit' ? 'block' : 'none' }}">
                            <label for="unit_id" class="block text-sm font-medium text-gray-700">Unit</label>
                            <select name="unit_id" id="unit_id" class="mt-1 block w-full rounded-md border-gray-300">
                                <option value="">Select unit</option>
                                @foreach($units as $u)
                                    <option value="{{ $u->id }}" {{ old('unit_id', $announcement->unit_id) == $u->id ? 'selected' : '' }}>{{ $u->code }} - {{ $u->name }}</option>
                                @endforeach
                            </select>
